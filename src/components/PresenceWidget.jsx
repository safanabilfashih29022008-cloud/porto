  ...
  useEffect(() => {
    const fetchPresence = async () => {
      try {
        ...
        const normalized = (data.activities || []).slice(0, 2).map((a, idx) => {
          if (a.type === "spotify") {
            return {
              key: `spotify-${idx}`,
              title: a.title,
              ...
              >
                <div className="p-3 flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    >
                      <div
                        className={`w-1 h-1 rounded-full ${colors.text.replace("text-", "bg-")} animate-pulse`}
                      ></div>
                      <span
                  ...
                  {act.type === "spotify" && (
                    <div className="">
                      <img
                        src="Spotify.png"
                        className="w-auto h-6 opacity-80 group-hover:opacity-100 transition-opacity duration-300"
                        alt=""
                      />
                    </div>
                  )}
